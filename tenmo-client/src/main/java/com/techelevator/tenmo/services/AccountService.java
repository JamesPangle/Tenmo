...
public class AccountService {
    ...
    public void requestMoney(int receiving, int sending, BigDecimal amountToRequest) {
        Account arequesting = getAccount(receiving);
        Account asending = getAccount(sending);
        if (asending == null || asending == arequesting) {
        ...
        if (amountToRequest.doubleValue() > 0) {
            Transfer newTransfer = new Transfer(1,
                    asending.getAccountId(), arequesting.getAccountId(), amountToRequest);
    ...
    public void acceptPendingRequest(Transfer transferToAcceptDecline, String isYesNo, int currentUserId) {
        ...
        if (isYesNo.equalsIgnoreCase("Yes") || isYesNo.equalsIgnoreCase("Y")) {
            ...
            Account receiving = getAccount(transferToAcceptDecline.getAccountTo());
            Account sending = getAccount(transferToAcceptDecline.getAccountFrom());
            if (currentUserId != receiving.getUserId()) {
                isAccepted = true;
                transferService.updateTransfer(transferToAcceptDecline);
            }
            transferService.processTransfer(receiving, sending,
